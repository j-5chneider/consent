...
# remotes::install_github("kroehne/ShinyItemBuilder", build_vignettes = F) 
...
prompt01 <- 'Please act like a language tutor and translate the "Input" into English language.

Only provide the translation as output in simple plain text. 

Input: "##INPUT##" 

' 

prompt02 <- 'Please act like a language tutor and correct spelling and grammar mistakes in the "Input". Please do mark words that are written in any other language than English in square brackets, but do not translate.
...
prompt03 <- 'Please act like a language tutor. Can you give one tip or advice how to improve the following input "Input" text that should fluent English?

Only provide the tip or advice as output in simple plain text. 
...
prompt04 <- 'Please act like a language tutor. Please rephrase the following text without grammar or spelling mistakes in advanced british English:  "##INPUT##".
...
item_pool <- getPool(path="ib/") 
assessment_config <- getConfig() 

# Interactive part (used while item is running)
  
assessment_config$interactive <- function(session, json){
  
  a <- jsonlite::fromJSON(json$data)
  
  task <- a$task

  print(task)
  
  if (task == "PROMPT_REQUEST"){
    
    participant_concern <- a$text
    requestedprompt <-  a$prompt
...
Please provide an honest assessment of the participant's concern(s), explaining how the study ensures data protection and addresses these specific issues. The response should be clear and informative to help participants understand the safeguards in place. Use straightforward and non-technical language to make the information accessible to all types of participants. Limit yourself strictly to answering the specific question, do not provide any additional information and do not offer any further help or to answer further questions. You do not have to introduce yourself or say goodbye.")
    ...
    if (run){
      ...
      tryCatch(
        expr = {
          message <- res$choices$message.content
          session$sendCustomMessage("shinyassess_process_response", 
                                    jsonlite::toJSON(list(task=task,
                                                          response=participant_concern,
                                                          message=message), 
                                                     pretty = T,  
                                                     auto_unbox = T))
        },
